...
namespace AirVinyl.API.DbContexts
{
    public class AirVinylDbContext : DbContext
    {
        ...
        {
            modelBuilder.Entity<DynamicProperty>().HasKey(d => new
            {
                d.Key,
                d.VinylRecordId
            });
            ...
            //https://github.com/dotnet/runtime/issues/58817
        ...
        public override Task<int> SaveChangesAsync(CancellationToken cancellationToken = default)
        {
            ...
            for (var i = 0; i < modifiedOrAddedVinylRecords.Count; i++)
            {
                ...
                foreach (var dynamicPropertyToSave in dynamicProperties)
                {
                    ...
                    var exitingDynamicProperty =
                        ChangeTracker.Entries<DynamicProperty>()
                        .FirstOrDefault(d => d.Entity.Key == dynamicPropertyToSave.Key);

                    if (exitingDynamicProperty != null)
                    {
                        DynamicVinylRecordProperties.Remove(exitingDynamicProperty.Entity);
                    }
